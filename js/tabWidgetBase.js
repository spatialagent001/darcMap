...
define(["dojo/_base/declare", "dijit/_WidgetBase", "dijit/form/Button", "dojo/_base/lang", "dojo/_base/Color", "dojo/dom-construct", "dojo/dom", "dojo/dom-class", "dojo/on", "dojo/_base/fx", "dojo/fx", "application/mobileHelper"], function(declare, _WidgetBase, Button, lang, Color, domConstruct, dom, domClass, on, baseFx, coreFx, MobileHelper) {
	...
	return declare('_tabWidgetBase', [_WidgetBase], {
		...
		postCreate : function() {
			...
			if (this.mobileHelper.clientWidth > (this.maxWidth+(this.containerWidthFactor + this.containerOverLap)))
		...
		initiateTab : function() {
			...
			this.containerTitle = domConstruct.create('div', {
				...
				style : "top:" + this.containerTopOffset + "px;height:30px;right:0px;",
			...
			this.cp = new dijit.layout.ContentPane({
				...
				content : "",
				style : "top:0px;height:" + (this.mobileHelper.clientHeight - ((this.containerTopOffset * 2) + 30)) + "px;width:" + (this.containerWidth-17) + "px;right:0px;"
			});
		...
		modifyTabSize : function(data) {
			...
			baseFx.animateProperty({
				node : this.baseClass + '_Container3',
				duration : 500,
				properties : {
					width : {
						start : data.previousWidth,
						end : (data.newWidth-17)
					}
				}
			})
			]).play();
